// Cloudflare Pages Function - Google Cloud TTS Proxy
...
const RATE_LIMIT = 40; // requests per window (per uid+ip)
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const ssml = typeof input?.ssml === 'string' ? input.ssml : '';
    const payloadSize = text.length + ssml.length + JSON.stringify({ voice, audioConfig }).length;

    if (!text && !ssml) {
      return new Response(
        JSON.stringify({ error: 'Missing input text/ssml' }),
