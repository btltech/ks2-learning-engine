// Cloudflare Pages Function - Parent child management (unlink)
// Unlink removes the relationship between parent and child:
// - removes childId from legacy `childrenIds` array (if present)
// - deletes users/{parentId}/children/{childId} link doc
// - sets users/{childId}.parentId = null
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (parentRole !== 'parent') {
      return jsonResponse(403, { error: 'Only parent accounts can unlink children' }, cors.headers);
    }
    ...
    if (childParentId !== parentUid) {
      return jsonResponse(403, { error: 'You are not allowed to unlink this child' }, cors.headers);
    }
        ...
        writes: [
          ...
          { delete: linkDocName },
          {
            update: {
              name: childDocName,
              fields: {
                parentId: firestoreNull(),
                updatedAt: firestoreTimestamp(now),
              },
            },
            updateMask: { fieldPaths: ['parentId', 'updatedAt'] },
          },
        ],
      }),
    });
    ...
    if (!resp.ok) {
      return jsonResponse(500, { error: data?.error?.message || 'Failed to unlink child' }, cors.headers);
    }

    return jsonResponse(200, { ok: true }, cors.headers);
  } catch (e: any) {
    return jsonResponse(500, { error: e?.message || 'Failed to unlink child' }, cors.headers);
  }
};
