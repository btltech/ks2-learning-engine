...
async function generateWeeklyStats(childId: string): Promise<WeeklyStats> {
  ...
  sessionsSnapshot.forEach((doc) => {
    ...
    const subject: string = session.subject ?? 'Unknown';
  ...
  for (const [subject, scores] of Object.entries(subjectScores)) {
    ...
    if (avg > highestAvg) { highestAvg = avg; strongestSubject = subject; }
    if (avg < lowestAvg) { lowestAvg = avg; weakestSubject = subject; }
  }
...
export const sendWeeklyReports = functions.pubsub
  ...
  .onRun(async () => {
    ...
    for (const parentDoc of parentsSnapshot.docs) {
      ...
      const parentName: string = parent.name ?? parent.displayName ?? 'Parent';
...
export const sendTestWeeklyReport = functions.https.onCall(async (data, context) => {
  ...
  const parentId: string = typeof data.parentId === 'string' ? data.parentId : context.auth.uid;
