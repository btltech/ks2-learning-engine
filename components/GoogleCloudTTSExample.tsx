    ...
    <div className="w-full max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
          ...
          {(['FEMALE', 'MALE'] as const).map(gender => (
            <label key={gender} className="flex items-center">
              <input
                ...
                onChange={e => setSelectedVoiceGender(e.target.value as 'MALE' | 'FEMALE')}
                disabled={isSpeaking || isLoading}
                className="w-4 h-4"
              />
      ...
      <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
        ...
        <ul className="text-sm text-gray-700 space-y-1">
          <li>• <strong>Web Speech API</strong>: Free, built-in browser text-to-speech</li>
          <li>• All 17 languages supported with native voices</li>
          <li>• Works offline in most browsers</li>
        </ul>
      </div>
